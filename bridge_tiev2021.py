...
def pub_navinfo_loop(interval, redis_client):
...
def pub_caninfo_loop(interval, redis_client):
	while True:
		if _stop_lcm:
			break
		start = time.process_time()
...
def publish_all_async(interval_high, interval_low):
	...
	t_pub_caninfo = threading.Thread(target=pub_caninfo_loop, args=(interval_high, rc))
...
def get_redis(pubsub):
	for item in pubsub.listen(): 
		if item['type'] == 'message':
				cancontrol_handler("CANCONTROL", item["data"])
