...
# origin_location (x,y,heading(rad))
origin_location = [201.5,-296.2,-1.58]
...
_tunnel = xcm.LCM("udpm://239.255.76.67:7667?ttl=1")
...
class KeyboardControl(object):
    ...
    def _parse_walker_keys(self, keys, milliseconds, world):
        ...
        if keys[K_DOWN] or keys[K_s]:
            ...
            self._rotation.yaw += 0.05*milliseconds
        if keys[K_LEFT] or keys[K_a]:
            self._control.speed = 0.01
            self._rotation.yaw -= 0.05 * milliseconds
        ...
        if keys[K_UP] or keys[K_w]:
            ...
            self._control.speed = 10
        if keys[K_UP] and pygame.key.get_mods() & KMOD_SHIFT:
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        self._info_text = [
            ...
            'heading: % 4.2f (deg)' % _navinfo.mHeading,
            'Accelero: (%5.1f,%5.1f,%5.1f)' % (world.imu_sensor.accelerometer),
            'Gyroscop: (%5.1f,%5.1f,%5.1f)' % (world.imu_sensor.gyroscope),
            'Location:% 20s' % ('(% 5.1f, % 5.1f)' % (t.location.x, t.location.y)),
            'GNSS(lat/lon):% 24s' % ('(% 2.6f, % 3.6f)' % (world.gnss_sensor.lat, world.gnss_sensor.lon)),
...
def main():
    ...
    argparser.add_argument(
        ...
        metavar='PATTERN',
        default='walker.*',
        help='actor filter (default: "walker.*")')
    argparser.add_argument(
        '--rolename',
        metavar='NAME',
        default='hero',
