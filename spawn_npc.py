...
def main():
    ...
    try:
        ...
        npc_points = json.load(f)['npc_points']
        for n, transform in enumerate(spawn_points):
            if n >= args.number_of_vehicles:
                break
            if n<len(npc_points):
                transform.location.x = npc_points[n][0]
                transform.location.y = npc_points[n][1]
                transform.location.z = npc_points[n][2] + 0.5
                transform.rotation.yaw = -math.degrees(npc_points[n][3])
            ...
            batch.append(SpawnActor(blueprint, transform)
                .then(SetAutopilot(FutureActor, args.autopilot, traffic_manager.get_port()))
